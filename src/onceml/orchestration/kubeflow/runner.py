...
class KubeflowRunner(BaseRunner):
    ...
    def _construct_pipeline_graph(self, pipeline: Pipeline):
        ...
        for component in pipeline.components:
            ...
            kfp_component=Kfp_component.KfpComponent(pipeline_root=pipeline.rootdir,component=component,depends_on=depends_on,Do_deploytype=Do_deploytype)
    ...
    def allocate_component_artifact_url(self,pipeline:Pipeline):
        ...
        for c in pipeline.components:
            if type(c)==GlobalComponent:
                ...
                os.symlink(src=os.path.join(os.getcwd(),self._output_dir,pipeline._task_name,c._alias_model_name,c._alias_component_id),dst=os.path.join(os.getcwd(),self._output_dir,pipeline.rootdir,c.id))
                ...
                if not os.path.exists(os.path.join(self._output_dir,pipeline.rootdir,c.id)):
                    ...
                    os.makedirs(os.path.join(self._output_dir,pipeline.rootdir,c.id),exist_ok=True)
                c.artifact.setUrl(os.path.join(pipeline.rootdir,c.id))
    def deploy(self, pipeline: Pipeline):
        ...
        kfp_ops.ensure_pipeline(self._kfp_client,os.path.join(self._output_dir,'yamls',file_name),pipeline)
        #对数据库中的信息进行更新
        pipeline.db_store()