...
OUTPUTSDIR = 'kfpoutputs'  # kfp输出目录，每个组件的输出会保存在这
...
COMPONENT_POD_LABEL = '{}.component.id'.format(global_config.project_name).lower()
...
COMPONENT_SENDER_POD_LABEL = '{tool}.{project}.{{task}}.{{model}}.{{component}}'.format(
    tool=global_config.project_name, project=global_config.PROJECTDIRNAME).lower()
# NFS容器相关
